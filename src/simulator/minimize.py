# Тип представления автомата (состояние, символы, переходы, выходы, начальное состояние)
Machine = tuple[list[str], list[str], dict[str, dict[str, str]], dict[str, str], str]

# Удаляем состояния, недостижимые из начального 
...
# Уточняем разбиение состояний на классы эквивалентности
...
# Выполняем минимизацию по алгоритму Мура
